...
실제 앱 반영 상태: **2026-09-14 Phase 8.5 완료**
...
- `시스템 설정`, `라이트`, `다크` 테마를 앱 Shell, File Tree, Editor, Database, Dialog와 상태 표시에 공통 적용했다.
- 시스템 테마 변경을 즉시 감지하고, 페이지가 그려지기 전에 저장된 테마를 먼저 적용해 초기 색상 전환을 최소화했다.
- 파랑, 주황, 보라, 검정/흰색 키 컬러와 저장 가능한 커스텀 HEX 컬러를 제공하며, 색상 밝기에 따라 Button 글자색을 자동 결정한다.
- 선택한 테마와 키 컬러는 기존 Preference Store를 통해 현재 브라우저에 유지된다.
- `Notion 기본`, `Pretendard`, `명조`, `고정폭` 문서 글꼴을 제공하고 Editor와 Database Content에만 적용했다.
- 새로 여는 문서의 기본 모드를 `에디터` 또는 `Markdown`으로 선택할 수 있으며, 이미 열린 문서의 현재 모드는 유지한다.
...
- 초기값은 `사용`, `3초`로 하며 1초·5초도 선택할 수 있다.
